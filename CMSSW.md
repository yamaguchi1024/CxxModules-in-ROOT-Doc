...
You can also use source from current directory instead of fetching from remote. You need to clone ROOT and add `--source root:Source=root-6.17.01`.

7. Build CMSSW

Successfully build cmssw-tool-conf? Congrats! Then we can finally build CMSSW!
...
Actuall rootcling invocation (genreflex command is just a wrapper of rootcling), which is executed by `_genreflex.py`
```
